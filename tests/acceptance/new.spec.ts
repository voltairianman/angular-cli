...
describe("Acceptance: ng new", function() {
    ...
    beforeEach(done => {
        ...
        spyOn(console, "error");
        // symlink custom collections to node_modules, so we can use with ng new it is a
        // bit dirty, but bootstrap-local tricks won't work here
        fs.symlinkSync(
            `${process.cwd()}/tests/collections/@custom`,
            `./node_modules/@custom`,
            "dir"
        );
    ...
    it("ng new with empty app does throw exception", done => {
        return ng(["new", ""]).then(() => done.fail(), () => done());
    });
    ...
    it("ng new with app name creates new directory and has a dasherized package name", done => {
        return ng(["new", "FooApp", "--skip-install", "--skip-git"])
            .then(() => {
    ...
    it("ng new with --dry-run does not create new directory", done => {
        return ng(["new", "foo", "--dry-run"])
            .then(() => {
                const cwd = process.cwd();
                expect(cwd).not.toMatch(
                    /foo/,
                    "does not change cwd to foo in a dry run"
                );
                expect(fs.pathExistsSync(path.join(cwd, "foo"))).toBe(
                    false,
                    "does not create new directory"
                );
                expect(fs.pathExistsSync(path.join(cwd, ".git"))).toBe(
                    false,
                    "does not create git in current directory"
                );
            })
            .then(done, done.fail);
    });
    ...
    it("ng new with --directory uses given directory name and has correct package name", done => {
        return ng([
            ...
            .then(() => {
                ...
                expect(cwd).not.toMatch(
                    /foo/,
                    "does not use app name for directory name"
                );
                expect(fs.pathExistsSync(path.join(cwd, "foo"))).toBe(
                    false,
                    "does not create new directory with app name"
                );

                expect(cwd).toMatch(/bar/, "uses given directory name");
                expect(fs.pathExistsSync(path.join(cwd, "..", "bar"))).toBe(
                    true,
                    "creates new directory with specified name"
                );
                ...
                expect(pkgJson.name).toBe(
                    "foo",
                    "uses app name for package name"
                );
            })
            .then(done, done.fail);
    });
    ...
    it("should specify a version of the CLI", done => {
        return ng(["new", "FooApp", "--skip-install", "--skip-git"])
            .then(() => {
                ...
                expect(pkgJson.devDependencies["@angular/cli"]).toMatch(
                    /\d+\.\d+\.\d+/
                );
            })
            .then(done, done.fail);
