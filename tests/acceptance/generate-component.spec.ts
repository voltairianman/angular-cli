...
describe("Acceptance: ng generate component", () => {
    ...
    it("test" + path.sep + "my-comp", done => {
        ...
        return ng(["generate", "component", "test" + path.sep + "my-comp"])
            .then(() => {
                const testPath = path.join(
                    root,
                    "tmp",
        ...
        return ng(["generate", "component", "mycomp"])
            .then(() => {
                const cliJson = JSON.parse(
                    readFileSync(".angular-cli.json", "utf8")
                );
                ...
                expect(
                    contents.indexOf(
                        `styleUrls: [\'./mycomp.component.${styleExt}\']`
                    ) === -1
                ).toBe(false);
            })
            .then(done, done.fail);
        ...
        return ng(["generate", "component", ".." + path.sep + "my-comp"])
            ...
            .catch(err => {
                expect(err).toBe(
                    `Invalid path: "..${
                        path.sep
                    }my-comp" cannot be above the "src${path.sep}app" directory`
                );
            })
            .then(done, done.fail);
    });
    ...
    it("mycomp will prefix selector", done => {
        return ng(["generate", "component", "mycomp"])
            .then(() => {
                ...
                expect(contents.indexOf("selector: 'app-mycomp'") === -1).toBe(
                    false
                );
            })
            .then(done, done.fail);
    ...
    it("mycomp --no-prefix will not prefix selector", done => {
        return ng(["generate", "component", "mycomp", "--no-prefix"])
            .then(() => {
                ...
                expect(pathExistsSync(testPath)).toBe(true);
                const contents = readFileSync(testPath, "utf8");
                expect(contents.indexOf("selector: 'mycomp'") === -1).toBe(
                    false
                );
            })
            .then(done, done.fail);
    ...
    it("mycomp --prefix=test will prefix selector with 'test-'", done => {
        return ng(["generate", "component", "mycomp", "--prefix=test"])
            .then(() => {
                ...
                expect(pathExistsSync(testPath)).toBe(true);
                const contents = readFileSync(testPath, "utf8");
                expect(contents.indexOf("selector: 'test-mycomp'") === -1).toBe(
                    false
                );
            })
            .then(done, done.fail);
    ...
    it(`non${path.sep}existing${path.sep}dir${
        path.sep
    }myComp will create dir and succeed`, done => {
        const testPath = path.join(
            root,
            "tmp",
            "foo",
        ...
        return ng([
            "generate",
            "component",
            `non${path.sep}existing${path.sep}dir${path.sep}myComp`,
        ])
            .then(() => expect(pathExistsSync(testPath)).toBe(true))
            .then(() => readFile(appModule, "utf-8"))
            .then(content => {
                // Expect that the app.module contains a reference to my-comp and its import.
                expect(content).toMatch(
                    /import.*MyCompComponent.*from '.\/non\/existing\/dir\/my-comp\/my-comp.component';/
                );
            })
            .then(done, done.fail);
    });
    ...
    it("my-comp --inline-template", done => {
        return ng(["generate", "component", "my-comp", "--inline-template"])
            .then(() => {
                const testPath = path.join(
                    root,
                    "tmp",
    ...
    it("my-comp --inline-style", done => {
        return ng(["generate", "component", "my-comp", "--inline-style"])
            .then(() => {
                const testPath = path.join(
                    root,
                    "tmp",
    ...
    it("my-comp --no-spec", done => {
        return ng(["generate", "component", "my-comp", "--no-spec"])
            .then(() => {
                const testPath = path.join(
                    root,
                    "tmp",
    ...
    describe("should import and add to declaration list", () => {
        ...
        it("when given a root level module with missing module.ts suffix", done => {
            ...
            return Promise.resolve()
                .then(() =>
                    ng(["generate", "component", "baz", "--module", "app"])
                )
                .then(() => readFile(modulePath, "utf-8"))
                .then(content => {
                    expect(content).toMatch(
        ...
        it("when given a submodule folder", done => {
            ...
            return Promise.resolve()
                .then(() => ng(["generate", "module", "foo"]))
                .then(() =>
                    ng(["generate", "component", "baz", "--module", "foo"])
                )
                .then(() => readFile(modulePath, "utf-8"))
                .then(content => {
                    expect(content).toMatch(
        ...
        describe("should generate components in apps with empty appRoot", () => {
            it("should work", done => {
                ...
                return ng(["generate", "module", "m", "--app", "other"])
                    .then(() => {
                        ...
                        return ng([
                            ...
                            expect(
                                readFileSync(expectedModule, "utf-8")
                            ).toContain(
