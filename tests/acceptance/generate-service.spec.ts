...
describe("Acceptance: ng generate service", () => {
    ...
    it("ng generate service my-svc", done => {
        ...
        const testSpecPath = path.join(
            appRoot,
            "src/app/my-svc.service.spec.ts"
        );
    ...
    it("ng generate service test" + path.sep + "my-svc", done => {
        ...
        return ng(["generate", "service", "test" + path.sep + "my-svc"])
            .then(() => {
    ...
    it(
        "ng generate service test" + path.sep + ".." + path.sep + "my-svc",
        done => {
            return ng([
                "generate",
                "service",
                "test" + path.sep + ".." + path.sep + "my-svc",
            ])
                .then(() => {
                    const testPath = path.join(
                        root,
    ...
    it(
        "ng generate service child-dir" + path.sep + "my-svc from a child dir",
        done => {
            fs.mkdirsSync(
            ...
            return new Promise(function(resolve) {
                ...
                .then(() => {
                    ...
                    return ng([
                        "generate",
                        "service",
                        "child-dir" + path.sep + ".." + path.sep + "my-svc",
                    ]);
                })
                .then(() => {
                    const testPath = path.join(
    ...
    it(
        "ng generate service " +
            path.sep +
            "my-svc from a child dir, gens under " +
            path.join("src", "app"),
        done => {
            fs.mkdirsSync(path.join(root, "tmp", "foo", "src", "app", "1"));
            return new Promise(function(resolve) {
    ...
    it(
        "ng generate service .." + path.sep + "my-svc from root dir will fail",
        done => {
            return ng(["generate", "service", ".." + path.sep + "my-svc"])
                .then(() => done.fail())
                .catch((err: string) => {
                    ...
                    expect(err).toBe(
                        `Invalid path: "..${
                            path.sep
                        }my-svc" cannot be above the "src${
                            path.sep
                        }app" directory`
                    );
                    done();
    ...
    describe("should import and add to provider list", () => {
        ...
        it("when given a root level module with missing module.ts suffix", done => {
            ...
            return Promise.resolve()
                .then(() =>
                    ng(["generate", "service", "baz", "--module", "app"])
                )
                .then(() => fs.readFile(modulePath, "utf-8"))
                .then(content => {
                    expect(content).toMatch(
        ...
        it("when given a submodule folder", done => {
            ...
            return Promise.resolve()
                .then(() => ng(["generate", "module", "foo"]))
                .then(() =>
                    ng(["generate", "service", "baz", "--module", "foo"])
                )
                .then(() => fs.readFile(modulePath, "utf-8"))
                .then(content => {
                    expect(content).toMatch(
