...
describe("Acceptance: ng generate guard", function() {
    ...
    it("ng generate guard my-guard", done => {
        ...
        const testSpecPath = path.join(
            appRoot,
            "src/app/my-guard.guard.spec.ts"
        );
    ...
    it("ng generate guard test" + path.sep + "my-guard", done => {
        ...
        return ng(["generate", "guard", "test" + path.sep + "my-guard"])
            .then(() => {
    ...
    it(
        "ng generate guard test" + path.sep + ".." + path.sep + "my-guard",
        done => {
            return ng([
                "generate",
                "guard",
                "test" + path.sep + ".." + path.sep + "my-guard",
            ])
                .then(() => {
                    const testPath = path.join(
                        root,
    ...
    it(
        "ng generate guard child-dir" + path.sep + "my-guard from a child dir",
        done => {
            fs.mkdirsSync(
            ...
            return new Promise(function(resolve) {
                ...
                .then(() => {
                    ...
                    return ng([
                        "generate",
                        "guard",
                        "child-dir" + path.sep + ".." + path.sep + "my-guard",
                    ]);
                })
                .then(() => {
                    const testPath = path.join(
    ...
    it(
        "ng generate guard " +
            path.sep +
            "my-guard from a child dir, gens under " +
            path.join("src", "app"),
        done => {
            fs.mkdirsSync(path.join(root, "tmp", "foo", "src", "app", "1"));
            return new Promise(function(resolve) {
    ...
    it(
        "ng generate guard .." + path.sep + "my-guard from root dir will fail",
        done => {
            return ng(["generate", "guard", ".." + path.sep + "my-guard"])
                .then(() => done.fail())
                .catch(err => {
                    ...
                    expect(err).toBe(
                        `Invalid path: "..${
                            path.sep
                        }my-guard" cannot be above the "src${
                            path.sep
                        }app" directory`
                    );
                })
            ...
            return Promise.resolve()
                ...
                .then(content => {
                    expect(content).toMatch(
                        /import.*BazGuard.*from '.\/baz.guard';/
                    );
