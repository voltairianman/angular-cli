...
describe("Acceptance: ng generate directive", () => {
        ...
        return ng(["generate", "directive", "flat"])
            .then(() => {
                const testPath = path.join(
                    root,
                    "tmp/foo/src/app/flat.directive.ts"
                );
    ...
    it("my-dir --flat false", done => {
        ...
        const testPath = path.join(
            appRoot,
            "src/app/my-dir/my-dir.directive.ts"
        );
        const testSpecPath = path.join(
            appRoot,
            "src/app/my-dir/my-dir.directive.spec.ts"
        );
    ...
    it(
        "child-dir" + path.sep + ".." + path.sep + "my-dir from a child dir",
        done => {
            fs.mkdirsSync(path.join(root, "tmp", "foo", "src", "app", "1"));
            return new Promise(function(resolve) {
                process.chdir("./src");
                ...
                .then(() => {
                    ...
                    return ng([
                        "generate",
                        "directive",
                        "child-dir" + path.sep + ".." + path.sep + "my-dir",
                        "--flat",
                        "false",
                    ]);
                })
    ...
    it(
        path.sep +
            "my-dir from a child dir, gens under " +
            path.join("src", "app"),
        done => {
            fs.mkdirsSync(path.join(root, "tmp", "foo", "src", "app", "1"));
            return new Promise(function(resolve) {
        ...
        return ng(["generate", "directive", ".." + path.sep + "my-dir"])
            ...
            .catch(err => {
                expect(err).toBe(
                    `Invalid path: "..${
                        path.sep
                    }my-dir" cannot be above the "src${path.sep}app" directory`
                );
            })
            .then(done, done.fail);
    });
    ...
    describe("should import and add to declaration list", () => {
        ...
        it("when given a root level module with missing module.ts suffix", done => {
            ...
            return Promise.resolve()
                .then(() =>
                    ng(["generate", "directive", "baz", "--module", "app"])
                )
                .then(() => fs.readFile(modulePath, "utf-8"))
                .then(content => {
                    expect(content).toMatch(
        ...
        it("when given a submodule folder", done => {
            ...
            return Promise.resolve()
                .then(() => ng(["generate", "module", "foo"]))
                .then(() =>
                    ng(["generate", "directive", "baz", "--module", "foo"])
                )
                .then(() => fs.readFile(modulePath, "utf-8"))
                .then(content => {
                    expect(content).toMatch(
