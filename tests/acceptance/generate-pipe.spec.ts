...
describe("Acceptance: ng generate pipe", () => {
    ...
    it("ng generate pipe test" + path.sep + "my-pipe", done => {
        ...
        return ng(["generate", "pipe", "test" + path.sep + "my-pipe"])
            .then(() => {
            ...
            return ng(["generate", "pipe", ".." + path.sep + "my-pipe"])
                ...
                .catch(err => {
                    ...
                    expect(err).toBe(
                        `Invalid path: "..${
                            path.sep
                        }my-pipe" cannot be above the "src${
                            path.sep
                        }app" directory`
                    );
                    done();
    ...
    describe("should import and add to declaration list", () => {
        it("when given a root level module with module.ts suffix", done => {
            ...
            return Promise.resolve()
                .then(() =>
                    ng(["generate", "pipe", "baz", "--module", "app.module.ts"])
                )
                .then(() => fs.readFile(modulePath, "utf-8"))
                .then(content => {
                    expect(content).toMatch(
                        /import.*BazPipe.*from '.\/baz.pipe';/
                    );
                    // tslint:disable-next-line:max-line-length
                    expect(content).toMatch(
                        /declarations:\s+\[\r?\n\s+AppComponent,\r?\n\s+BazPipe\r?\n\s+\]/m
