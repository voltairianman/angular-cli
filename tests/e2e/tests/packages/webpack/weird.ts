...
export default function(skipCleaning: () => void) {
    return (
        Promise.resolve()
            ...
            .then(() =>
                expectFileToMatch("dist/app.main.js", /PRODUCTION_ONLY/)
            )
            .then(() =>
                expectToFail(() =>
                    expectFileToMatch("dist/app.main.js", /DEBUG_ONLY/)
                )
            )

            // Skip code generation and rebuild.
            .then(() =>
                updateJsonFile("aotplugin.config.json", json => {
                    json["skipCodeGeneration"] = true;
                })
            )
            .then(() =>
                updateJsonFile("webpack.flags.json", json => {
                    json["DEBUG"] = true;
                })
            )
            .then(() => exec(normalize("node_modules/.bin/webpack"), "-p"))
            ...
            .then(() =>
                expectToFail(() =>
                    expectFileSizeToBeUnder("dist/app.main.js", 410000)
                )
            )
            .then(() => expectFileSizeToBeUnder("dist/0.app.main.js", 40000))
