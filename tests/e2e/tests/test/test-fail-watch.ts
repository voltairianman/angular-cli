...
// Karma is only really finished with a run when it shows a non-zero total time
// in the first slot.
        ...
        execAndWaitForOutputToMatch("ng", ["test"], karmaGoodRegEx)
            ...
            .then(() =>
                writeFile(
                    ...
                    "<p> definitely not typescript </p>"
                )
            )
            .then(() =>
                expectToFail(() =>
                    waitForAnyProcessOutputToMatch(karmaGoodRegEx, 10000)
                )
            )
