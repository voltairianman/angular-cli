    ...
    return (
        Promise.resolve()
            .then(() =>
                writeMultipleFiles({
                    "src/styles.css":
                        "div { background: url('./assets/more.png'); }",
                })
            )
            // use image with file size >10KB to prevent inlining
            .then(() =>
                copyProjectAsset("images/spectrum.png", "./assets/more.png")
            )
            .then(() => ng("build", "--deploy-url=deployUrl/", "--extract-css"))
            .then(() =>
                expectFileToMatch("dist/index.html", "deployUrl/main.bundle.js")
            )
            ...
            .then(() =>
                ng(
                    "build",
                    "--deploy-url=http://example.com/some/path/",
                    "--extract-css"
                )
            )
            .then(() =>
                expectFileToMatch(
                    "dist/index.html",
                    "http://example.com/some/path/main.bundle.js"
                )
            )
            ...
            .then(() =>
                expectFileToMatch(
                    "dist/index.html",
                    "config-deployUrl/main.bundle.js"
                )
            )
            // verify --deploy-url is applied to non-extracted css urls
            .then(() =>
                ng("build", "--deploy-url=deployUrl/", "--extract-css=false")
            )
            .then(() =>
                expectFileToMatch(
