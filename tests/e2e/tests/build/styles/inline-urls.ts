...
export default function() {
    return (
      ...
      `,
            ...
            .then(() =>
                copyProjectAsset("images/spectrum.png", "./assets/large.png")
            )
            .then(() =>
            ...
            .then(({ stdout }) => {
                if (
                    stdout.match(
                        /postcss-url: \.+: Can't read file '\.+', ignoring/
                    )
                ) {
                    throw new Error(
                        "Expected no postcss-url file read warnings."
                    );
                }
            })
            ...
            .then(() =>
                expectFileMatchToExist("./dist", /large\.[0-9a-f]{20}\.png/)
            )
            .then(() =>
                expectFileMatchToExist("./dist", /small-id\.[0-9a-f]{20}\.svg/)
            )
    );
}
