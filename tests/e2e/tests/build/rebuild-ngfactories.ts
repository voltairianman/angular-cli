...
export default function() {
    ...
    return (
        execAndWaitForOutputToMatch("ng", ["serve", "--aot"], validBundleRegEx)
            .then(() =>
                writeMultipleFiles({
      ...
      `,
            ...
            // Trigger a few rebuilds first. The AOT compiler is still optimizing rebuilds
            ...
            .then(body => {
                if (!body.match(/HTML_REBUILD_STRING/)) {
                    throw new Error(
                        "Expected HTML_REBUILD_STRING but it wasn't in bundle."
                    );
                }
            })
            ...
            .then(body => {
                if (!body.match(/CSS_REBUILD_STRING/)) {
                    throw new Error(
                        "Expected CSS_REBUILD_STRING but it wasn't in bundle."
                    );
                }
            })
            ...
            .then(body => {
                if (!body.match(/CSS_DEP_REBUILD_STRING/)) {
                    throw new Error(
                        "Expected CSS_DEP_REBUILD_STRING but it wasn't in bundle."
                    );
                }
            })
            .then(() => {
                ...
                if (
                    getGlobalVariable("argv").ng2 ||
                    getGlobalVariable("argv").ng4
                ) {
                    return Promise.resolve();
                }
                ...
                return Promise.resolve()
                    ...
                    .then(body => {
                        if (!body.match(/FACTORY_REBUILD_STRING/)) {
                            throw new Error(
                                "Expected FACTORY_REBUILD_STRING but it wasn't in bundle."
                            );
                        }
                    });
            })
