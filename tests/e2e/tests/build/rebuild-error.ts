...
export default function() {
    ...
    return (
        Promise.resolve()
            ...
            .then(() =>
                execAndWaitForOutputToMatch("ng", ["serve", "--aot"], failedRe)
            )
            .then(results => {
                ...
                if (
                    ...
                    throw new Error(
                        `Expected static analysis error, got this instead:\n${stderr}`
                    );
                }
                if (extraErrors.some(e => stderr.includes(e))) {
                    throw new Error(
                        `Did not expect extra errors but got:\n${stderr}`
                    );
                }
            })
            ...
            .then(() => wait(2000))
            // Add an syntax error to a non-main file. Build should still be successfull and
            // error reported on forked type checker.
            .then(() =>
                Promise.all([
            ...
            .then(results => {
                ...
                if (!stderr.includes("Declaration or statement expected.")) {
                    throw new Error(
                        `Expected syntax error, got this instead:\n${stderr}`
                    );
                }
                if (extraErrors.some(e => stderr.includes(e))) {
                    throw new Error(
            ...
            // Fix the error, should trigger a successful rebuild. We have to wait for the
            // type checker to run, so we expect to NOT have an error message in 5s.
            .then(() =>
                Promise.all([
                    expectToFail(() =>
                        waitForAnyProcessOutputToMatch(errorRe, 5000)
                    ),
                    writeFile("src/app/app.component.ts", origContent),
                ])
            )
            .then(() => wait(2000))
            // Add a major static analysis error on a rebuild. Should fail the rebuild.
            .then(() =>
                Promise.all([
