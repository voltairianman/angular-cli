...
const doneRe = /webpack: bundle is now VALID|webpack: Compiled successfully.|webpack: Failed to compile./;
...
export default function() {
    ...
    `
            ...
            // Make an invalid version of the file. Should trigger a rebuild, this time an
            // error is expected.
            .then(() =>
                Promise.all([
      ...
      `
            ...
            // Change an UNRELATED file and the error should still happen. Should trigger a
            // rebuild, this time an error is also expected.
            .then(() =>
                Promise.all([
                    waitForAnyProcessOutputToMatch(errorRe, 20000),
                    appendToFile(
      ...
      `
            ...
            .then(results => {
                ...
                if (!/ERROR in (.*src\/)?main\.ts/.test(stderr)) {
                    throw new Error(
                        "Expected an error to still be there but none was."
                    );
                }
            })
            // Fix the error!
      ...
      `
            ...
            .then(results => {
                ...
                if (/ERROR in (.*src\/)?main\.ts/.test(stderr)) {
                    throw new Error(
                        "Expected no error but an error was shown."
                    );
                }
            })
            .then(
