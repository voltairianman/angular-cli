...
export default function() {
    // @filipesilva: This test doesn't work correctly on CircleCI while being ran by
    // the test script. Polling time seems to be ignored and several builds are
    // fired per second. Debugging showed that webpack things the `src/` directory
    // changed on each rebuild. Disabling for now.
    ...
    return (
        execAndWaitForOutputToMatch(
            ...
            // Wait before editing a file. Editing too soon seems to trigger a rebuild and
            // throw polling out of whack.
            .then(() => wait(3000))
            ...
            .then(() =>
                expectToFail(() =>
                    waitForAnyProcessOutputToMatch(webpackGoodRegEx, 7000)
                )
            )
