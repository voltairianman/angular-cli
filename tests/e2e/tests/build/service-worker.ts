...
export default function() {
    ...
    // Can't use the `ng` helper because somewhere the environment gets stuck to the
    // first build done
    return silentNpm("remove", "@angular/service-worker")
        ...
        .then(() =>
            writeFile("src/ngsw-config.json", JSON.stringify(MANIFEST, null, 2))
        )
        .then(() => ng("build", "--prod"))
        ...
        .then(() => expectFileToExist(join(process.cwd(), "dist/ngsw.json")))
        .then(() =>
            expectFileToMatch("dist/ngsw.json", /"\/foo\/bar\/index.html"/)
        )
        .then(() => ng("build", "--prod", "--service-worker=false"))
        .then(() => expectFileNotToExist("dist/ngsw.json"))
        .then(() =>
            writeFile(
                "node_modules/@angular/service-worker/safety-worker.js",
                "false"
            )
        )
        .then(() => ng("build", "--prod"))
        ...
        .then(() =>
            expectFileToExist(join(process.cwd(), "dist/ngsw-worker.js"))
        )
        .then(() => expectFileToExist(join(process.cwd(), "dist/ngsw.json")))
