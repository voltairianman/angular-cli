...
export default function() {
    ...
    return silentNpm("remove", "@angular/service-worker")
        .then(() =>
            silentNpm("install", "@angular/service-worker@1.0.0-beta.16")
        )
        .then(() => ng("set", "apps.0.serviceWorker=true"))
        .then(() => ng("build", "--prod"))
        .then(() => expectFileToExist(join(process.cwd(), "dist")))
        .then(() =>
            expectFileToExist(join(process.cwd(), "dist/ngsw-manifest.json"))
        )
        ...
        .then(() =>
            moveFile(
                rootManifest,
                join(process.cwd(), "src/ngsw-manifest.json")
            )
        )
        .then(() => ng("build", "--prod"))
        .then(() => expectFileToMatch("dist/ngsw-manifest.json", /\"local\"/))
