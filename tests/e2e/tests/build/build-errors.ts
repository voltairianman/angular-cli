...
export default function() {
    ...
    return (
        Promise.resolve()
            ...
            // Check `part of the TypeScript compilation` errors. These should show an error
            // only for the missing file.
            .then(() =>
                updateJsonFile("./src/tsconfig.app.json", configJson => {
            ...
            .then(({ message }) => {
                if (
                    !message.includes(
                        "polyfills.ts is missing from the TypeScript compilation"
                    )
                ) {
                    throw new Error(
                        `Expected missing TS file error, got this instead:\n${message}`
                    );
                }
            ...
            // Check simple single syntax errors. These shouldn't skip emit and just show a
            // TS error.
