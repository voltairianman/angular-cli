...
export default function() {
    ...
    return (
        execAndWaitForOutputToMatch("ng", ["serve"], validBundleRegEx)
            ...
            // Should trigger a rebuild with a new bundle. We need to use Promise.all to
            // ensure we are waiting for the rebuild just before we write the file,
            // otherwise rebuilds can be too fast and fail CI.
            .then(() =>
                Promise.all([
      ...
      `
            ...
            .then(results => {
                ...
                if (!lazyChunkRegExp.test(stdout)) {
                    throw new Error(
                        "Expected webpack to create a new chunk, but did not."
                    );
                }
            })
            // Change multiple files and check that all of them are invalidated and
            // recompiled.
            .then(() =>
                Promise.all([
                    waitForAnyProcessOutputToMatch(validBundleRegEx, 10000),
