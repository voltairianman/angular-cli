...
function _recursiveMkDir(path: string): Promise<void> {
    if (fs.existsSync(path)) {
        ...
        return _recursiveMkDir(dirname(path)).then(() => fs.mkdirSync(path));
    }
}

export function copyFile(from: string, to: string) {
    return _recursiveMkDir(dirname(to)).then(
        () =>
...
export function writeMultipleFiles(fs: { [path: string]: string }) {
    return Promise.all(
        Object.keys(fs).map(fileName => writeFile(fileName, fs[fileName]))
    );
}
...
export function expectFileMatchToExist(dir: string, regex: RegExp) {
    return new Promise((resolve, reject) => {
        ...
        if (!fileName) {
            reject(
                new Error(
                    `File ${regex} was expected to exist but not found...`
                )
            );
        }
        resolve(fileName);
    ...
    return new Promise((resolve, reject) => {
        fs.exists(fileName, exist => {
            if (exist) {
                reject(
                    new Error(
                        `File ${fileName} was expected not to exist but found...`
                    )
                );
            } else {
                resolve();
    ...
    return new Promise((resolve, reject) => {
        fs.exists(fileName, exist => {
            if (exist) {
                ...
                reject(
                    new Error(
                        `File ${fileName} was expected to exist but not found...`
                    )
                );
            }
        });
    ...
    return readFile(fileName).then(content => {
        if (typeof regEx == "string") {
            if (content.indexOf(regEx) == -1) {
                throw new Error(
                    stripIndents`File "${fileName}" did not contain "${regEx}"...
            Content:
            ${content}
            ------
          `
...
export function expectFileSizeToBeUnder(fileName: string, sizeInBytes: number) {
    return readFile(fileName).then(content => {
        if (content.length > sizeInBytes) {
            throw new Error(
                `File "${fileName}" exceeded file size of "${sizeInBytes}".`
            );
        }
    });
}
