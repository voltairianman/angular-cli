...
export function waitForAnyProcessOutputToMatch(
    ...
    // Race between _all_ processes, and the timeout. First one to resolve/reject
    // wins.
    const timeoutPromise: Promise<ProcessOutput> = new Promise(
        (_resolve, reject) => {
            // Wait for 30 seconds and timeout.
            setTimeout(() => {
                reject(
                    new Error(
                        `Waiting for ${match} timed out (timeout: ${timeout}msec)...`
                    )
                );
            }, timeout);
        }
...
export function execAndWaitForOutputToMatch(
    ...
    if (cmd === "ng" && args[0] === "serve") {
        // Accept matches up to 20 times after the initial match. Useful because the
        // Webpack watcher can rebuild a few times due to files changes that happened
        // just before the build (e.g. `git clean`). This seems to be due to host file
        // system differences, see https://nodejs.org/docs/latest/api/fs.html#fs_caveats
            ...
            .concat(
                Observable.defer(() =>
                    Observable.fromPromise(
                        waitForAnyProcessOutputToMatch(match, 2500)
                    )
                        .repeat(20)
            ...
            return maybeSilentNg("eject", ...args.slice(1), "--force")
                .then(() => {
                    if (!npmInstalledEject) {
                        ...
                        return rimraf("node_modules").then(() =>
                            silentNpm("install")
                        );
                    }
                })
