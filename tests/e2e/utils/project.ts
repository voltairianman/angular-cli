...
export function ngServe(...args: string[]) {
    return execAndWaitForOutputToMatch(
        "ng",
        ["serve", ...args],
...
export function createProject(name: string, ...args: string[]) {
    ...
    return Promise.resolve()
        ...
        .then(
            () =>
                argv.nightly || argv["ng-sha"] ? useSha() : Promise.resolve()
        )
            ...
            for (const packageName of Object.keys(devkitPackages)) {
                if (json["dependencies"].hasOwnProperty(packageName)) {
                    json["dependencies"][packageName] =
                        devkitPackages[packageName].tar;
                } else if (
                    json["devDependencies"].hasOwnProperty(packageName)
                ) {
...
export function useCIDefaults() {
    return updateJsonFile(".angular-cli.json", configJson => {
        // Auto-add some flags to ng commands that build or test the app. --no-progress
        // disables progress logging, which in CI logs thousands of lines.
        // --no-sourcemaps disables sourcemaps, making builds faster. We add these flags
        // before other args so that they can be overriden. e.g. `--no-sourcemaps
        // --sourcemaps` will still generate sourcemaps.
...
export function useCIChrome() {
    // There's a race condition happening in Chrome. Enabling logging in chrome used
    // by protractor actually fixes it. Logging is piped to a file so it doesn't
    // affect our setup. --no-sandbox is needed for Circle CI. Travis can use
    // headless chrome, but not appveyor.
    return (
        Promise.resolve()
            .then(() =>
                replaceInFile(
...
export function useNg2() {
    ...
    return Promise.resolve()
        ...
        .then(() =>
            updateJsonFile("src/tsconfig.app.json", json =>
                Object.assign(json, tsconfigAppJson)
            )
        )
        .then(() =>
            updateJsonFile("src/tsconfig.spec.json", json =>
                Object.assign(json, tsconfigSpecJson)
            )
        )
        .then(() =>
            updateJsonFile("e2e/tsconfig.e2e.json", json =>
