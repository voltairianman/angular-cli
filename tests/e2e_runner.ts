// This may seem awkward but we're using Logger in our e2e. At this point the
// unit tests have run already so it should be "safe", teehee.
import {
    ConsoleLoggerStack,
    LogEntry,
    IndentLogger,
...
const allSetups = glob
    ...
    .sort();
const allTests = glob
    .sync(path.join(e2eRoot, testGlob), {
        nodir: true,
        ignore: argv.ignore,
    })
    .map(name => path.relative(e2eRoot, name))
...
const shardedTests = tests.filter(
    (name, i) => shardId === null || i % nbShards == shardId
);
...
if (testsToRun.length == allTests.length) {
    ...
    console.log(
        `Running ${testsToRun.length} tests (${allTests.length +
            allSetups.length} total)`
    );
}
    ...
    .reduce((previous, relativeName, testIndex) => {
        ...
        return previous.then(() => {
            ...
            return Promise.resolve()
                ...
                .then(() => console.log("----"))
                .then(() => {
                    // If we're not in a setup, change the directory back to where it was before the
                    // test. This allows tests to chdir without worrying about keeping the original
                    // directory.
                ...
                .then(() => {
                    // Only clean after a real test, not a setup step. Also skip cleaning if the
                    // test requested an exception.
    ...
    .then(
        ...
        err => {
            ...
            if (argv.debug) {
                ...
                console.log(
                    "Will loop forever while you debug... CTRL-C to quit."
                );
...
function printHeader(testName: string, testIndex: number) {
    ...
    const fullIndex =
        (testIndex < allSetups.length
            ? testIndex
            : (testIndex - allSetups.length) * nbShards +
              shardId +
              allSetups.length) + 1;
    ...
    const shard =
        shardId === null
            ...
            : yellow(
                  ` [${shardId}:${nbShards}]` +
                      bold(` (${fullIndex}/${length})`)
              );
    console.log(
        green(
            `Running "${bold(blue(testName))}" (${bold(
                white(text)
            )}${shard})...`
        )
    );
