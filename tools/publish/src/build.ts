...
export default function build(
    ...
    return Promise.resolve()
        ...
        .then(() => {
            ...
            fs.writeFileSync(output, buildSchema(input, logger), {
                encoding: "utf-8",
            });
        })
        ...
        .then(() => {
            ...
            // Order packages in order of dependency. We use bubble sort because we need a
            // full topological sort but adding another dependency or implementing a full
            // topo sort would be too much work and I'm lazy. We don't anticipate any large
            // number of
            ...
            do {
                ...
                for (let i = 0; i < sortedPackages.length - 1; i++) {
                    for (let j = i + 1; j < sortedPackages.length; j++) {
                        ...
                        if (
                            Object.keys(getDeps(packages[a])).indexOf(b) != -1
                        ) {
                            // Swap them.
            ...
            return files
                ...
                .filter(fileName => {
                    ...
                    if (/\.ts$/.test(fileName)) {
                        ...
                        if (
                            !fs.existsSync(
                                path.join(dist, fileName).replace(/ts$/, "js")
                            )
                        ) {
                            throw new Error(
        ...
        .then((specFiles: string[]) =>
            specFiles.filter(fileName => {
                return !/[\\\/]@angular[\\\/]cli[\\\/]blueprints/.test(
                    fileName
                );
            })
        )
        ...
        .then(() => {
            ...
            return Promise.all(
                extraFiles.map(fileName => {
                    ...
                    return copy(
                        fileName,
                        path.join("dist/@angular/cli", fileName)
                    );
                })
            );
        })
        ...
        .then(() => {
            ...
            logger.info(
                "Changing dependencies between packages to tar files..."
            );
            logger.warn("=================================================");
            ...
            Object.keys(packages).forEach(pkgName => {
                ...
                const json = JSON.parse(
                    fs.readFileSync(pkg.packageJson).toString()
                );
                ...
                if (opts.devkit) {
                    ...
                    const devkitPackages = require(opts.devkit +
