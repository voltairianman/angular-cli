...
export abstract class SchemaTreeNode<T> implements SchemaNode {
    ...
    get description() {
        return "description" in this._schema
            ? this._schema["description"]
            : null;
    }
    get required() {
        if (!this._parent) {
        ...
        const type: string =
            "oneOf" in schema
                ...
                : "enum" in schema
                    ...
                    : schema["type"];
        let Klass: {
            new (arg: TreeNodeConstructorArgument<any>): SchemaTreeNode<any>;
        } = null;
...
export class OneOfSchemaTreeNode extends NonLeafSchemaTreeNode<any> {
    ...
    get defined() {
        return this._currentTypeHolder
            ? this._currentTypeHolder.defined
            : false;
    }
    get items() {
        ...
        if (schema["properties"]) {
            for (const name of Object.keys(schema["properties"])) {
                ...
                this._children[name] = this._createChildProperty(
                    ...
                    forward
                        ? (forward as ObjectSchemaTreeNode).children[name]
                        : null,
                    propertySchema
                );
            }
            ...
            throw new InvalidSchema(
                "Schema does not have a properties, but doesnt allow for additional properties."
            );
        }
...
export class ArraySchemaTreeNode extends NonLeafSchemaTreeNode<Array<any>> {
    ...
    constructor(metaData: TreeNodeConstructorArgument<Array<any>>) {
        ...
        // Keep the item's schema as a schema node. This is important to keep type
        // information.
        ...
        if (convertedValue === null || convertedValue === undefined) {
            if (this.required) {
                throw new InvalidValueError(
                    `Invalid value "${v}" on a required field.`
                );
            }
        }
...
class EnumSchemaTreeNode extends LeafSchemaTreeNode<any> {
    ...
    set(value: string, init = false, force = false) {
        if (!(value === undefined || this._isInEnum(value))) {
            throw new InvalidUpdateValue(
                "Invalid value can only be one of these: " + this.items
            );
        }
