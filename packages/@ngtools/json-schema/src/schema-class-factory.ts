...
class SchemaClassBase<T> implements SchemaClass<T> {
    ...
    $$alias(source: string, destination: string) {
        let sourceSchemaTreeNode = _getSchemaNodeForPath(
            this.$$schema(),
            source
        );
    ...
    $$set(path: string, value: any): void {
        ...
        if (node) {
            ...
            // This might be inside an object that can have additionalProperties, so a
            // TreeNode would not exist.
            ...
            const parent: any = splitPath
                .slice(0, -1)
                .reduce(
                    (parent: any, curr: string) => parent && parent[curr],
                    this
                );
