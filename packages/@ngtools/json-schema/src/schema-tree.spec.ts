...
describe("@ngtools/json-schema", () => {
    describe("OneOfSchemaTreeNode", () => {
        ...
        const schemaJson = JSON.parse(
            readFileSync(schemaJsonFilePath, "utf-8")
        );
        ...
        it("returns undefined for values that are non-existent", () => {
            const proto: any = Object.create(null);
            const root = new RootSchemaTreeNode(proto, {
                value: valueJson,
                schema: schemaJson,
            });

            const value = root.children["objectKey1"].children[
                "objectKey"
            ].children["stringKey"].get();
        ...
        it("should throw error when setting invalid value", () => {
            ...
            try {
                ...
                expect(error.message).toBe(
                    "Invalid value can only be one of these: v1,v2,v3"
                );
            }
        });
    });
