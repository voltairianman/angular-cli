...
export const NodeWatchFileSystem: NodeWatchFileSystemInterface = require("webpack/lib/node/NodeWatchFileSystem");
...
export class VirtualFileSystemDecorator implements InputFileSystem {
    ...
    // We only need to intercept calls to individual files that are present in
    // WebpackCompilerHost.
    ...
    purge(changes?: string[] | string): void {
        if (typeof changes === "string") {
            ...
            changes.forEach((fileName: string) =>
                this._webpackCompilerHost.invalidate(fileName)
            );
        }
...
export class VirtualWatchFileSystemDecorator extends NodeWatchFileSystem {
    ...
    watch(
        ...
        const newCallback = (
            ...
            const virtualFilesStats = this._virtualInputFileSystem
                .getVirtualFilesPaths()
                .map(fileName => ({
                    path: fileName,
                    mtime: +this._virtualInputFileSystem.statSync(fileName)
                        .mtime,
                }));
            virtualFilesStats.forEach(
                stats => (fileTimestamps[stats.path] = +stats.mtime)
            );
            callback(
                err,
                filesModified,
