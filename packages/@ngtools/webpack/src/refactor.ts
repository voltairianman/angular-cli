...
export function findAstNodes<T extends ts.Node>(
    ...
    // TODO: refactor operations that only need `refactor.findAstNodes()` to use
    // this instead.
    if (max == 0) {
        return [];
    }
    ...
    if (max > 0) {
        for (const child of node.getChildren(sourceFile)) {
            findAstNodes(child, sourceFile, kind, recursive, max).forEach(
                (node: ts.Node) => {
                    if (max > 0) {
                        arr.push(node as T);
                    }
...
export class TypeScriptFileRefactor {
    ...
    getDiagnostics(typeCheck = true): ts.Diagnostic[] {
        ...
        // only concat the declaration diagnostics if the tsconfig config sets it to
        // true.
        if (this._program.getCompilerOptions().declaration == true) {
            diagnostics = diagnostics.concat(
                this._program.getDeclarationDiagnostics(this._sourceFile)
            );
        }
        diagnostics = diagnostics.concat(
    ...
    insertImport(symbolName: string, modulePath: string): void {
        // Find all imports.
        const allImports = this.findAstNodes(
            this._sourceFile,
        ...
        const maybeImports = allImports
            ...
            .filter((node: ts.ImportDeclaration) => {
                // Remove import statements that are either `import 'XYZ'` or `import * as X
                // from 'XYZ'`.
            ...
            .map((node: ts.ImportDeclaration) => {
                ...
                return (node.importClause as ts.ImportClause)
                    .namedBindings as ts.NamedImports;
            });

        if (maybeImports.length) {
            // There's an `import {A, B, C} from 'modulePath'`. Find if it's in either
            // imports. If so, just return; nothing to do.
            const hasImportAlready = maybeImports.some(
    ...
    removeNode(node: ts.Node) {
        this._sourceString.remove(
            node.getStart(this._sourceFile),
            node.getEnd()
        );
        this._changed = true;
    }
    ...
    transpile(compilerOptions: ts.CompilerOptions): TranspileOutput {
        ...
        if (result.sourceMapText) {
            ...
            return { outputText: result.outputText, sourceMap: null };
        }
    }
}
