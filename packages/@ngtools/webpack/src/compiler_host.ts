...
export class WebpackCompilerHost implements ts.CompilerHost {
    ...
    private _files: {
        [path: string]: VirtualFileStats | null;
    } = Object.create(null);
    private _directories: {
        [path: string]: VirtualDirStats | null;
    } = Object.create(null);
    ...
    constructor(private _options: ts.CompilerOptions, basePath: string) {
        ...
        this._delegate = ts.createCompilerHost(
            this._options,
    ...
    getNgFactoryPaths(): string[] {
        return (
            Object.keys(this._files)
                .filter(
                    fileName =>
                        fileName.endsWith(".ngfactory.js") ||
                        fileName.endsWith(".ngstyle.js")
                )
                // These paths are used by the virtual file system decorator so we must
                // denormalize them.
    ...
    fileExists(fileName: string, delegate = true): boolean {
        ...
        return (
            this._files[fileName] != null ||
            (delegate && this._delegate.fileExists(fileName))
        );
    }
    ...
    directoryExists(directoryName: string, delegate = true): boolean {
        ...
        return (
            this._directories[directoryName] != null ||
            (delegate &&
    ...
    getSourceFile(
        ...
        if (stats == null) {
            ...
            if (!this._cache) {
                ...
                // If cache is turned on and the file exists, the readFile call will have
                // populated stats. Empty stats at this point mean the file doesn't exist at and
    ...
    // This is due to typescript CompilerHost interface being weird on writeFile.
    // This shuts down typings in WebStorm.
    get writeFile() {
        return (
            fileName: string,
            data: string,
