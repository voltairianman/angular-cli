...
describe("@ngtools/webpack", () => {
    describe("loader", () => {
        ...
        `,
                ...
                expect(refactor.sourceText).not.toMatch(
                    /\{\s*otherValue: 1\s*};/
                );

                const refactor2 = new TypeScriptFileRefactor(
                    "/file2.ts",
                    host,
                    program
                );
                ...
                expect(refactor2.sourceText).toMatch(
                    /\(\{\s*otherValue1: 1\s*}\)/
                );
                expect(refactor2.sourceText).toMatch(
                    /\(\{\s*otherValue2: 2\s*,\s*otherValue3: 3\s*}\)/
                );
                expect(refactor2.sourceText).toMatch(
                    /\(\{\s*otherValue4: 4\s*}\)/
                );
