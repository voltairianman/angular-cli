...
// Manually check for Compiler CLI availability and supported version. This is
// needed because @ngtools/webpack does not depend directly on
// @angular/compiler-cli, since it is installed as part of global Angular CLI
// installs and compiler-cli is not of its dependencies.
export function CompilerCliIsSupported() {
    ...
    // Check that Angular is also not part of this module's node_modules (it should
    // be the project's).
    const compilerCliPath = require.resolve("@angular/compiler-cli");
    if (compilerCliPath.startsWith(path.dirname(__dirname))) {
        throw new Error(
            "The @ngtools/webpack plugin now relies on the project @angular/compiler-cli. P" +
                "lease clean your node_modules and reinstall."
        );
    }
...
// These imports do not exist on a global install for Angular CLI, so we cannot
// use a static ES6 import.
let compilerCli: any = {};
try {
    compilerCli = require("@angular/compiler-cli");
} catch (e) {
    // Don't throw an error if the private API does not exist. Instead, the
    // `CompilerCliIsSupported` method should return throw and indicate the plugin
    // cannot be used.
}
...
// These imports do not exist on Angular versions lower than 5, so we cannot use
// a static ES6 import.
let ngtools2: any = {};
try {
    ...
    // Don't throw an error if the private API does not exist. Instead, the
    // `AngularCompilerPlugin.isSupported` method should return false and indicate
    // the plugin cannot be used.
}

export const createProgram: CreateProgramInterface = ngtools2.createProgram;
export const createCompilerHost: CreateCompilerHostInterface =
    ngtools2.createCompilerHost;
export const formatDiagnostics: FormatDiagnosticsInterface =
    ngtools2.formatDiagnostics;
export const readConfiguration: ReadConfigurationInterface =
