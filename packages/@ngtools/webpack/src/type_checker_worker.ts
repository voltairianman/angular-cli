    ...
    process.on("message", (message: TypeCheckerMessage) => {
        ...
        switch (message.kind) {
            ...
            case MESSAGE_KIND.Update:
                if (!typeChecker) {
                    throw new Error(
                        "TypeChecker: update message received before initialization"
                    );
                }
                if (lastCancellationToken) {
                    // This cancellation token doesn't seem to do much, messages don't seem to be
                    // processed before the diagnostics finish.
                ...
                typeChecker.update(
                    updateMessage.rootNames,
                    updateMessage.changedCompilationFiles,
                    lastCancellationToken
                );
                break;
            default:
                throw new Error(
                    `TypeChecker: Unexpected message received: ${message}.`
                );
        }
