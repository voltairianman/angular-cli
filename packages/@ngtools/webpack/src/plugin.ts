...
export class AotPlugin implements Tapable {
    ...
    get moduleResolutionCache() {
        return this._moduleResolutionCache;
    }
    ...
    private _setupOptions(options: AotPluginOptions) {
        ...
        if (!options.hasOwnProperty("tsConfigPath")) {
            throw new Error(
                "Must specify 'tsConfigPath' in the configuration of @ngtools/webpack."
            );
        }
        // TS represents paths internally with '/' and expects the tsconfig path to be
        // in this format
        ...
        if (options.hasOwnProperty("exclude")) {
            // If the tsconfig doesn't contain any excludes, we must add the default ones
            // before adding any extra ones (otherwise we'd include all of these which can
            // cause unexpected errors). This is the same logic as present in TypeScript.
        ...
        // Check the genDir. We generate a default gendir that's under basepath; it will
        // generate a `node_modules` directory and because of that we don't want
        // TypeScript resolution to resolve to that directory but the real
        // `node_modules`.
        ...
        if (this._angularCompilerOptions.hasOwnProperty("genDir")) {
            genDir = path.resolve(
                basePath,
                this._angularCompilerOptions.genDir
            );
        ...
        if (options.hostOverrideFileSystem) {
            for (const filePath of Object.keys(
                options.hostOverrideFileSystem
            )) {
                this._compilerHost.writeFile(
                    filePath,
                    options.hostOverrideFileSystem[filePath],
                    false
                );
            }
        }
        ...
        if (options.hostReplacementPaths) {
            for (const filePath of Object.keys(options.hostReplacementPaths)) {
                const replacementFilePath =
                    options.hostReplacementPaths[filePath];
                const content = this._compilerHost.readFile(
                    replacementFilePath
                );
        ...
        // We enable caching of the filesystem in compilerHost _after_ the program has
        // been created, because we don't want SourceFile instances to be cached past
        // this point.
        ...
        if (options.entryModule) {
            ...
            (tsConfig.raw["angularCompilerOptions"] as any) &&
            (tsConfig.raw["angularCompilerOptions"] as any).entryModule
        ) {
            this._entryModule = path.resolve(
                this._basePath,
                (tsConfig.raw["angularCompilerOptions"] as any).entryModule
            );
        }
        ...
        if (options.hasOwnProperty("missingTranslation")) {
            const [MAJOR, MINOR, PATCH] = VERSION.full
                .split(".")
                .map((x: string) => parseInt(x, 10));
            if (MAJOR < 4 || (MINOR == 2 && PATCH < 2)) {
                console.warn(
                    `The --missing-translation parameter will be ignored because it is only ` +
    ...
    private _findLazyRoutesInAst(): LazyRouteMap {
        ...
        for (const filePath of changedFilePaths) {
            const fileLazyRoutes = findLazyRoutes(
                filePath,
                this._compilerHost,
                this._program
            );
    ...
    private _getLazyRoutesFromNgtools() {
        try {
            ...
            // We silence the error that the @angular/router could not be found. In that
            // case, there is basically no route supported by the app itself.
            if (
                err.message.startsWith(
                    "Could not resolve module @angular/router"
                )
            ) {
                return {};
            } else {
    ...
    apply(compiler: any) {
        // Decorate inputFileSystem to serve contents of CompilerHost. Use decorated
        // inputFileSystem in watchFileSystem.
        ...
        compiler.plugin("invalid", () => {
            // Turn this off as soon as a file becomes invalid and we're about to start a
            // rebuild.
        ...
        compiler.plugin("context-module-factory", (cmf: any) => {
            ...
            // Pick the last part after the last node_modules instance. We do this to let
            // people have a linked @angular/core or cli which would not be under the same
            // path as the project being built.
            const angularCoreModuleDir = path
                .dirname(angularCoreModulePath)
                .split(/node_modules/)
                .pop();
            ...
            if (angularCorePackageJson["es2015"]) {
                const angularCoreEs2015Path = path.resolve(
                    path.dirname(angularCorePackagePath),
                    angularCorePackageJson["es2015"]
                );
                angularCoreEs2015Dir = path
                    .dirname(angularCoreEs2015Path)
                    .split(/node_modules/)
                    .pop();
            }
            ...
            cmf.plugin(
                ...
                (result: any, callback: (err?: any, request?: any) => void) => {
                    ...
                    // @angular/core/src/linker matches for 2.*.*,   The other logic is for flat
                    // modules and requires reading the package.json of angular     (see above).
                    if (
                        !result.resource.endsWith(
                            path.join("@angular/core/src/linker")
                        ) &&
                        !(
                            angularCoreModuleDir &&
                            result.resource.endsWith(angularCoreModuleDir)
                        ) &&
                        !(
                            angularCoreEs2015Dir &&
                            result.resource.endsWith(angularCoreEs2015Dir)
                        )
                    ) {
        ...
        compiler.plugin("make", (compilation: any, cb: any) =>
        ...
        compiler.plugin("after-resolvers", (compiler: any) => {
            // Virtual file system. Wait for the plugin to be done when requesting `.ts`
            // files directly (entry points), or when the issuer is a `.ts` file.
            ...
            diagnostics.forEach(diagnostic => {
                ...
                if (diagnostic.file) {
                    ...
                    let {
                        ...
                        character,
                        fileName,
                    } = this._translateSourceMap(
                        sourceText,
                        diagnostic.file.fileName,
                        position
                    );

                    message = `${fileName} (${line + 1},${character +
                        1}): ${messageText}`;
                } else {
                    message = messageText;
                }
    ...
    private _make(compilation: any, cb: (err?: any, request?: any) => void) {
        ...
        this._donePromise = Promise.resolve()
                ...
                return __NGTOOLS_PRIVATE_API_2
                    .codeGen({
                        ...
                        readResource: (path: string) =>
                            this._resourceLoader.get(path),
                    })
                    .then(() => timeEnd("AotPlugin._make.codeGen"));
            })
            .then(() => {
                // Get the ngfactory that were created by the previous step, and add them to the
                // root file path (if those files exists).
                const newRootFilePath = this._compilerHost
                    .getChangedFilePaths()
                ...
                // Create a new Program, based on the old one. This will trigger a resolution of
                // all transitive modules, which include files that might just have been
                // generated. This needs to happen after the code generator has been created for
                // generated files to be properly resolved.
            ...
            .then(() => {
                // We need to run the `listLazyRoutes` the first time because it also navigates
                // libraries and other things that we might miss using the findLazyRoutesInAst.
                ...
                Object.keys(this.discoveredLazyRoutes).forEach(k => {
                    ...
                    if (this.skipCodeGeneration) {
                        ...
                        const factoryPath = lazyRoute.replace(
                            /(\.d)?\.ts$/,
                            ".ngfactory.ts"
                        );
                        const lr = path.relative(this.basePath, factoryPath);
                        this._lazyRoutes[k + ".ngfactory"] = path.join(
                            this.genDir,
                            lr
                        );
                    }
