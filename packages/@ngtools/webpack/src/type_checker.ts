...
// Force basic color support on terminals with no color support. Chalk typings
// don't have the correct constructor parameters.
...
export class UpdateMessage extends TypeCheckerMessage {
    constructor(
        public rootNames: string[],
        public changedCompilationFiles: string[]
    ) {
...
export class TypeChecker {
    ...
    constructor(
        ...
        // We don't set a async resource loader on the compiler host because we only
        // support html templates, which are the only ones that can throw errors, and
        // those can be loaded synchronously. If we need to also report errors on styles
        // then we'll need to ask the main thread for these resources.
