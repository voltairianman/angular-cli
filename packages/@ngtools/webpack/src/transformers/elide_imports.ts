...
// Remove imports for which all identifiers have been removed. Needs type
// checker, and works even if it's not the first transformer. Works by removing
// imports for symbols whose identifiers have all been removed. Doesn't use the
// `symbol.declarations` because that previous transforms might have removed
// nodes but the type checker doesn't know. See
// https://github.com/Microsoft/TypeScript/issues/17552 for more information.
export function elideImports(
    ...
    ts.forEachChild(sourceFile, function visit(node) {
        ...
        if (ts.isIdentifier(node)) {
            ...
            // Export specifiers return the non-local symbol from the above so check the
            // name string instead
            specialCaseNames.add((node.propertyName || node.name).text);
            return;
        } else if (ts.isShorthandPropertyAssignment(node)) {
            // Shorthand property assignments return the object property's symbol not the
            // import's
        ...
        if (node.importClause.name) {
            ...
            for (const specifier of node.importClause.namedBindings.elements) {
                if (isUnused(specifier.propertyName || specifier.name)) {
                    specifierOps.push(
                        new RemoveNodeOperation(sourceFile, specifier)
                    );
                }
            }
