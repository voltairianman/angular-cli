    ...
    return (
        ...
        const transformer: ts.Transformer<ts.SourceFile> = (
            ...
            const addOps = ops.filter(
                op => op.kind === OPERATION_KIND.Add
            ) as AddNodeOperation[];
            const replaceOps = ops.filter(
                op => op.kind === OPERATION_KIND.Replace
            ) as ReplaceNodeOperation[];

            // If nodes are removed, elide the imports as well. Mainly a workaround for
            // https://github.com/Microsoft/TypeScript/issues/17552. WARNING: this assumes
            // that replaceOps DO NOT reuse any of the nodes they are replacing. This is
            // currently true for transforms that use replaceOps (replace_bootstrap and
            // replace_resources), but may not be true for new transforms.
            if (getTypeChecker && removeOps.length + replaceOps.length > 0) {
                const removedNodes = removeOps
                    .concat(replaceOps)
                    .map(op => op.target);
                removeOps.push(
                    ...elideImports(sf, removedNodes, getTypeChecker)
                );
            }
            ...
            if (
                removeOps.some(op => op.target.kind === ts.SyntaxKind.Decorator)
            ) {
...
function visitEachChildWorkaround(
    ...
    if (node.kind === ts.SyntaxKind.SourceFile) {
        ...
        const statements = ts.visitLexicalEnvironment(
            sf.statements,
            visitor,
            context
        );
...
// 1) If TS sees an empty decorator array, it will still emit a `__decorate`
// call.    This seems to be a TS bug. 2) Also ensure nodes with modified
// decorators have parents    built in TS transformers assume certain nodes have
// parents (fixed in TS 2.7+)
