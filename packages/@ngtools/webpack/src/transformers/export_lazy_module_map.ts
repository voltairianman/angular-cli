...
export function exportLazyModuleMap(
    ...
    const standardTransform: StandardTransform = function(
        ...
        const modules = Object.keys(lazyRoutes).map(loadChildrenString => {
            const [, moduleName] = loadChildrenString.split("#");
            const modulePath = lazyRoutes[loadChildrenString];

            return { modulePath, moduleName, loadChildrenString };
        });

        modules.forEach((module, index) => {
            const relativePath = path
                .relative(dirName, module.modulePath!)
                .replace(/\\/g, "/");
            // Create the new namespace import node.
            const namespaceImport = ts.createNamespaceImport(
                ts.createIdentifier(`__lazy_${index}__`)
            );
            ...
            modules.map((mod, idx) => {
                ...
                return ts.createPropertyAssignment(
                    ...
                    ts.createPropertyAccess(
                        ts.createIdentifier(`__lazy_${idx}__`),
                        mod.moduleName
                    )
                );
            })
