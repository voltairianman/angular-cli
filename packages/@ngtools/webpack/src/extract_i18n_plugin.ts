...
export class ExtractI18nPlugin implements Tapable {
    ...
    private _setupOptions(options: ExtractI18nPluginOptions) {
        ...
        try {
            tsConfigJson = JSON.parse(
                fs.readFileSync(this._tsConfigPath, "utf8")
            );
        } catch (err) {
            throw new Error(
        ...
        if (options.hasOwnProperty("exclude")) {
            ...
            exclude.forEach((pattern: string) => {
                ...
                fileNames = fileNames.filter(
                    x => !x.replace(/\\/g, "/").match(re)
                );
            });
        } else {
            fileNames = fileNames.filter(
                fileName => !/\.spec\.ts$/.test(fileName)
            );
        }
        ...
        if (options.hasOwnProperty("locale")) {
            if (VERSION.major === "2") {
                console.warn(
                    "The option '--locale' is only available on the xi18n command starting from Ang" +
                        "ular v4, please update to a newer version.",
                    "\n\n"
                );
            }
        ...
        if (options.hasOwnProperty("outFile")) {
            if (VERSION.major === "2") {
                console.warn(
                    "The option '--out-file' is only available on the xi18n command starting from A" +
                        "ngular v4, please update to a newer version.",
                    "\n\n"
                );
            }
    ...
    private _make(compilation: any, cb: (err?: any, request?: any) => void) {
        ...
        if (this._compilation._ngToolsWebpackXi18nPluginInstance) {
            return cb(
                new Error(
                    "An @ngtools/webpack xi18n plugin already exist for this compilation."
                )
            );
        }
        if (!this._compilation._ngToolsWebpackPluginInstance) {
            return cb(
                new Error(
                    "An @ngtools/webpack aot plugin does not exists for this compilation"
                )
            );
        }
