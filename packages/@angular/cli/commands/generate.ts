...
import {
    DynamicPathOptions,
    dynamicPathParser,
} from "../utilities/dynamic-path-parser";
...
export default Command.extend({
    ...
    getCollectionName(
        ...
        let collectionName: string = CliConfig.getValue(
            "defaults.schematics.collection"
        );
      ...
      `)
        ...
        if (/^\d/.test(rawArgs[1])) {
            SilentError.debugOrThrow(
                "@angular/cli/commands/generate",
                `The \`ng generate ${schematicName} ${
                    rawArgs[1]
                }\` file name cannot begin with a digit.`
            );
        }

        const SchematicGetOptionsTask = require("../tasks/schematic-get-options")
            .default;
        ...
        return getOptionsTask
            .run({ schematicName, collectionName })
            .then((availableOptions: SchematicAvailableOptions[]) => {
                let anonymousOptions: string[] = [];

                if (availableOptions) {
                    const nameOption = availableOptions.filter(
                        opt => opt.name === "name"
                    )[0];
                    if (nameOption) {
    ...
    run: function(commandOptions: any, rawArgs: string[]) {
        ...
        if (entityName) {
            commandOptions.name = stringUtils.dasherize(
                entityName.split(separatorRegEx).pop()
            );
        } else {
            entityName = "";
        }
        ...
        commandOptions.sourceDir = parsedPath.sourceDir.replace(
            separatorRegEx,
            "/"
        );
        const root = parsedPath.sourceDir + path.sep;
        commandOptions.appRoot =
            parsedPath.appRoot === parsedPath.sourceDir
                ? ""
        ...
        commandOptions.path =
            parsedPath.dir === parsedPath.sourceDir
                ? ""
        ...
        const [collectionName, schematicName] = this.getCollectionName(
            rawArgs,
            commandOptions
        );

        if (
            ["component", "c", "directive", "d"].indexOf(schematicName) !== -1
        ) {
            ...
            if (schematicName === "component" || schematicName === "c") {
                if (commandOptions.styleext === undefined) {
                    commandOptions.styleext = CliConfig.getValue(
                        "defaults.styleExt"
                    );
                }
            }
        ...
        const loggerSubscription = logger
    ...
    printDetailedHelp: function(
        ...
        if (schematicName) {
            const SchematicGetHelpOutputTask = require("../tasks/schematic-get-help-output")
                .default;
            const getHelpOutputTask = new SchematicGetHelpOutputTask({
                ui: this.ui,
                project: this.project,
            });
            return getHelpOutputTask
                .run({
                    schematicName,
                    collectionName,
                    nonSchematicOptions: this.availableOptions.filter(
                        (o: any) => !o.hidden
                    ),
                })
                .then((output: string[]) => {
                    return [
                        cyan(
                            `ng generate ${schematicName} ${cyan(
                                "[name]"
                            )} ${cyan("<options...>")}`
                        ),
                        ...output,
            ...
            const schematicNames: string[] = engineHost.listSchematics(
                collection
            );
            const output: string[] = [];
