...
export const baseBuildCommandOptions: any = [
    ...
    },
    {
        ...
        default:
            typeof buildConfigDefaults["progress"] !== "undefined"
    ...
    },
    {
        ...
        default: "none",
        description:
            "Available on server platform only. Which external dependencies to bundle into " +
    ...
    },
    {
        ...
        aliases: ["sw"],
        description:
            "Generates a service worker config for production builds, if the app has servic" +
            "e worker enabled.",
    },
    {
        name: "skip-app-shell",
...
const BuildCommand = Command.extend({
    ...
    run: function(commandOptions: BuildTaskOptions) {
        ...
        if (
            commandOptions.target === "development" &&
            commandOptions.watch === true
        ) {
            commandOptions.forceTsCommonjs = true;
        }

        // Add trailing slash if missing to prevent
        // https://github.com/angular/angular-cli/issues/7295
        if (
            commandOptions.deployUrl &&
            commandOptions.deployUrl.substr(-1) !== "/"
        ) {
            commandOptions.deployUrl += "/";
        }
        ...
        const buildTask = new BuildTask({ project: this.project, ui: this.ui });

        const clientApp = getAppFromConfig(commandOptions.app);

        const doAppShell =
            commandOptions.target === "production" &&
        ...
        return buildPromise
            ...
            .then(() => {
                const RenderUniversalTask = require("../tasks/render-universal")
                    .default;
