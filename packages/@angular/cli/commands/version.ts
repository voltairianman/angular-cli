    ...
    run: function(_options: any) {
        ...
        if (!__dirname.match(/node_modules/)) {
            ...
            try {
                const gitRefName =
                    "" +
                    child_process.execSync("git symbolic-ref HEAD", {
                        cwd: __dirname,
                    });
        ...
        if (projPkg) {
            roots.forEach(root => {
                versions = Object.assign(
                    versions,
                    this.getDependencyVersions(projPkg, root)
                );
            });
            ...
            if (angularCoreVersion) {
                for (const angularPackage of Object.keys(angular)) {
                    if (angular[angularPackage] == angularCoreVersion) {
                        angularSameAsCore.push(
                            angularPackage.replace(/^@angular\//, "")
                        );
...
/_/   \\_\\_| |_|\\__, |\\__,_|_|\\__,_|_|       \\____|_____|___|
    ...
    ${Object.keys(angular)
        .map(module => module + ": " + angular[module])
        .sort()
        .join("\n")}
    ...
    getDependencyVersions: function(
        pkg: any,
        prefix: string
    ): {
        [name: string]: string;
        ...
        if (prefix[0] == "@") {
            try {
                fs.readdirSync(
                    path.resolve(this.project.root, "node_modules", prefix)
                )
