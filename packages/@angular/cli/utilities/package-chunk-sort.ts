...
// Sort chunks according to a predefined order: inline, polyfills, all styles,
// vendor, main
export function packageChunkSort(appConfig: any) {
    ...
    if (appConfig.styles) {
        extraEntryParser(appConfig.styles, "./", "styles").forEach(
            pushExtraEntries
        );
    }

    if (appConfig.scripts) {
        extraEntryParser(appConfig.scripts, "./", "scripts").forEach(
            pushExtraEntries
        );
    }
    ...
    // We need to list of entry points for the Ejected webpack config to work (we
    // reuse the function defined above).(sort as any).entryPoints = entryPoints;
    return sort;
}
