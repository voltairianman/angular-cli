...
class InitialCalculator extends Calculator {
    calculate() {
        const initialChunks = this.compilation.chunks.filter(
            chunk => chunk.isInitial
        );
    ...
    calculate() {
        return Object.keys(this.compilation.assets)
            ...
            .map(key => {
                ...
                return { size: asset.size(), label: key };
            });
    }
}
...
class AnyCalculator extends Calculator {
    calculate() {
        return Object.keys(this.compilation.assets).map(key => {
            const asset = this.compilation.assets[key];
            return { size: asset.size(), label: key };
        });
    }
...
function calculatePercentBytes(
    val: string,
    baseline?: string,
    factor?: "pos" | "neg"
): number {
    ...
    return (
        baselineBytes +
        ((baselineBytes * percentage) / 100) * (factor === "pos" ? 1 : -1)
    );
}
