...
export function checkYarnOrCNPM() {
    ...
    return Promise.all([checkYarn(), checkCNPM()]).then(
        (data: Array<boolean>) => {
            const [isYarnInstalled, isCNPMInstalled] = data;
            if (isYarnInstalled && isCNPMInstalled) {
                console.log(
                    chalk.yellow(
                        "You can `ng set --global packageManager=yarn` or `ng set --global packageManag" +
                            "er=cnpm`."
                    )
                );
                ...
                if (packageManager !== "default" && packageManager !== "npm") {
                    console.log(
                        chalk.yellow(
                            `Seems that ${packageManager} is not installed.`
                        )
                    );
                    console.log(
                        chalk.yellow(
                            "You can `ng set --global packageManager=npm`."
                        )
                    );
                }
            }
...
function checkYarn() {
    return execPromise("yarn --version").then(() => true, () => false);
}

function checkCNPM() {
    return execPromise("cnpm --version").then(() => true, () => false);
}
