...
export class CliConfig extends CliConfigBase<ConfigInterface> {
    ...
    static globalConfigFilePath(): string {
        ...
        const altGlobalConfigPath = path.join(
            homedir(),
            CLI_CONFIG_FILE_NAME_ALT
        );
    ...
    static fromProject(projectPath?: string): CliConfig {
        ...
        if (
            !configPath ||
            (configPath === this.globalConfigFilePath() &&
                process.cwd() !== path.dirname(configPath))
        ) {
            return null;
        }
    ...
    static addAliases(cliConfig: CliConfigBase<ConfigInterface>) {
      ...
      `)
        ...
        cliConfig.alias(
            "defaults.interface.prefix",
            "defaults.inline.prefixInterfaces"
        );
        cliConfig.alias(
            "defaults.component.inlineStyle",
            "defaults.inline.style"
        );
        cliConfig.alias(
            "defaults.component.inlineTemplate",
            "defaults.inline.template"
        );
        cliConfig.alias("defaults.component.spec", "defaults.spec.component");
        cliConfig.alias("defaults.class.spec", "defaults.spec.class");
        cliConfig.alias(
            "defaults.component.directive",
            "defaults.spec.directive"
        );
