...
export class NgCliWebpackConfig<T extends BuildOptions = BuildOptions> {
    ...
    constructor(buildOptions: T, appConfig: any) {
        ...
        const supportES2015 =
            tsConfig.options.target !== projectTs.ScriptTarget.ES3 &&
            tsConfig.options.target !== projectTs.ScriptTarget.ES5;

        // Only force if the default module kind is used (ES2015) Allows user to adjust
        // module kind (to esNext for example)
        buildOptions.forceTsCommonjs =
            buildOptions.forceTsCommonjs &&
    ...
    public buildConfig() {
        const platformConfig =
            this.wco.appConfig.platform === "server"
    ...
    public addTargetDefaults(buildOptions: T): T {
        const targetDefaults: {
            [target: string]: Partial<BuildOptions>;
        } = {
            development: {
                environment: "dev",
                outputHashing: "media",
        ...
        let merged = Object.assign(
            {},
            targetDefaults[buildOptions.target],
            buildOptions
        );

        // Use Build Optimizer on prod AOT builds by default when AngularCompilerPlugin
        // is supported.
