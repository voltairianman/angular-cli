...
function _createAotPlugin(
    ...
    // Forcing commonjs seems to drastically improve rebuild speeds on webpack. Dev
    // builds on watch mode will set this option to true.
    ...
    if (appConfig.environments) {
        ...
        if (!(buildOptions.environment in appConfig.environments)) {
            throw new SilentError(
                `Environment "${buildOptions.environment}" does not exist.`
            );
        }
...
export function getAotConfig(wco: WebpackConfigOptions) {
    ...
    const testTsConfigPath = path.resolve(
        projectRoot,
        appConfig.root,
        appConfig.testTsconfig
    );
    ...
    // Fallback to exclude spec files from AoT compilation on projects using a
    // shared tsconfig.
...
export function getNonAotTestConfig(wco: WebpackConfigOptions) {
    ...
    const appTsConfigPath = path.resolve(
        projectRoot,
        appConfig.root,
        appConfig.tsconfig
    );
    ...
    if (AngularCompilerPlugin.isSupported()) {
        if (appConfig.polyfills) {
            // TODO: remove singleFileIncludes for 2.0, this is just to support old projects
            // that did not include 'polyfills.ts' in `tsconfig.spec.json'.
        ...
        // The options below only apply to AoTPlugin. Force include main and polyfills.
        // This is needed for AngularCompilerPlugin compatibility with existing
        // projects, since TS compilation there is stricter and tsconfig.spec.ts doesn't
        // include them.
