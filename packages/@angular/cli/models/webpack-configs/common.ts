...
export function getCommonConfig(wco: WebpackConfigOptions) {
    ...
    if (appConfig.scripts.length > 0) {
        ...
        const globalScriptsByEntry = globalScripts.reduce(
            (
                prev: {
                    entry: string;
                    paths: string[];
    ...
    if (appConfig.assets) {
        const copyWebpackPluginPatterns = appConfig.assets.map(
            (asset: string | AssetPattern) => {
                ...
                asset =
                    typeof asset === "string"
                        ...
                        : asset;
                // Add defaults. Input is always resolved relative to the appRoot.
                asset.input = path
                    .resolve(appRoot, asset.input || "")
                    .replace(/\\/g, "/");
                asset.output = asset.output || "";
                asset.glob = asset.glob || "";

                // Prevent asset configurations from writing outside of the output path, except
                // if the user specify a configuration flag. Also prevent writing outside the
                // project path. That is not overridable.
                const absoluteOutputPath = path.resolve(
                    projectRoot,
                    buildOptions.outputPath
                );
                const absoluteAssetOutput = path.resolve(
                    absoluteOutputPath,
                    asset.output
                );
                const outputRelativeOutput = path.relative(
                    absoluteOutputPath,
                    absoluteAssetOutput
                );

                if (
                    outputRelativeOutput.startsWith("..") ||
                    path.isAbsolute(outputRelativeOutput)
                ) {
                    const projectRelativeOutput = path.relative(
                        projectRoot,
                        absoluteAssetOutput
                    );
                    if (
                        projectRelativeOutput.startsWith("..") ||
                        path.isAbsolute(projectRelativeOutput)
                    ) {
                        const message =
                    ...
                    if (!asset.allowOutsideOutDir) {
                        const message =
                            "An asset cannot be written to a location outside of the output path. You can o" +
                            "verride this message by setting the `allowOutsideOutDir` property on the asset" +
                            " to true in the CLI configuration.";
                        throw new SilentError(message);
                    }
                }

                // Prevent asset configurations from reading files outside of the project.
                const projectRelativeInput = path.relative(
                    projectRoot,
                    asset.input
                );
                if (
                    projectRelativeInput.startsWith("..") ||
                    path.isAbsolute(projectRelativeInput)
                ) {
                    const message =
        ...
        (copyWebpackPluginInstance as any)[
            "copyWebpackPluginPatterns"
        ] = copyWebpackPluginPatterns;
        (copyWebpackPluginInstance as any)[
            "copyWebpackPluginOptions"
    ...
    if (buildOptions.buildOptimizer) {
        // Set the cache directory to the Build Optimizer dir, so that package updates
        // will delete it.
        const buildOptimizerDir = path.dirname(
            resolve.sync("@angular-devkit/build-optimizer", {
                basedir: projectRoot,
            })
        );
    ...
    try {
        ...
        const rxPaths = requireProjectModule(
            projectRoot,
            rxjsPathMappingImport
        );
    ...
    return {
        ...
        plugins: [new webpack.NoEmitOnErrorsPlugin()].concat(extraPlugins),
    };
}
