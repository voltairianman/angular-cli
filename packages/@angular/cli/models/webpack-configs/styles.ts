...
const PostcssCliResources = require("../../plugins/webpack")
    .PostcssCliResources;
...
export function getStylesConfig(wco: WebpackConfigOptions) {
    ...
    const postcssPluginCreator = function(
        ...
        return [
            ...
            postcssUrl([
                {
                    ...
                    // require().
                    filter: ({ url }: PostcssUrlAsset) =>
                        url.startsWith("/") && !url.startsWith("//"),
                    url: ({ url }: PostcssUrlAsset) => {
                        if (
                            deployUrl.match(/:\/\//) ||
                            deployUrl.startsWith("/")
                        ) {
                            // If deployUrl is absolute or root relative, ignore baseHref & use deployUrl as
                            // is.
                            ...
                            // Join together base-href, deploy-url and the original URL. Also dedupe
                            // multiple slashes into single ones.
            ...
            PostcssCliResources({
                deployUrl:
                    loader.loaders[loader.loaderIndex].options.ident ==
                    "extracted"
                        ? ""
                        : deployUrl,
                loader,
    ...
    if (appConfig.styles.length > 0) {
        const globalStyles = extraEntryParser(
            appConfig.styles,
            appRoot,
            "styles"
        );
        ...
        rules.push(
            ...baseRules.map(({ test, use }) => {
                const extractTextPlugin = {
                    ...
                    // publicPath needed as a workaround
                    // https://github.com/angular/angular-cli/issues/4035
                    publicPath: "",
                };
                const ret: any = {
