...
export const Extracti18nTask = Task.extend({
    run: function(runTaskOptions: any) {
        ...
        // We need to determine the outFile name so that AngularCompiler can retrieve
        // it.
        let outFile =
            runTaskOptions.outFile || getI18nOutfile(runTaskOptions.i18nFormat);
        if (useExperimentalAngularCompiler && runTaskOptions.outputPath) {
            // AngularCompilerPlugin doesn't support genDir so we have to adjust outFile
            // instead.
