...
export const E2eTask = Task.extend({
    run: function(e2eTaskOptions: E2eTaskOptions) {
        ...
        const protractorLauncher = requireProjectModule(
            projectRoot,
        ...
        if (appConfig.platform === "server") {
            throw new SilentError(
                "ng test for platform server applications is coming soon!"
            );
        }
        ...
        return new Promise(function() {
            ...
            `
                ...
                // run `webdriver-manager update --standalone false --gecko false --quiet` if
                // you change this, update the command comment in prev line, and in `eject` task
                promise = promise.then(() =>
                    webdriverUpdate.program.run({
                        standalone: false,
                        gecko: false,
                        quiet: true,
                    })
            ...
            return promise.then(() =>
                protractorLauncher.init(
                    e2eTaskOptions.config,
