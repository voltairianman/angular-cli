...
export default Task.extend({
    run: function(commandOptions: any, _rawArgs: string[]) {
        ...
        const packageName =
            (commandOptions.name !== "." && commandOptions.name) ||
            project.name();

        if (commandOptions.style === undefined) {
            commandOptions.style = CliConfig.fromGlobal().get(
                "defaults.styleExt"
            );
        }
        ...
        if (!packageName) {
            const message =
                "The `ng " +
                this.name +
                "` command requires a package.json in " +
                "current folder with name attribute or a specified name via arguments. For more" +
                " details, use `ng help`.";
        ...
        const schematicName = CliConfig.fromGlobal().get(
            "defaults.schematics.newApp"
        );
        ...
        return schematicRunTask
            ...
            .then(() => {
                if (!commandOptions.dryRun) {
                    ...
                    this.ui.writeLine(
                        chalk.green(
                            `Project '${packageName}' successfully created.`
                        )
                    );
                }
            });
