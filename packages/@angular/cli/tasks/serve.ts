...
function findDefaultServePath(
    ...
    // normalize baseHref for ng serve the starting base is always `/` so a relative
    ...
    if (deployUrl && deployUrl[0] === "/") {
        if (
            baseHref &&
            baseHref[0] === "/" &&
            normalizedBaseHref !== deployUrl
        ) {
            // If baseHref and deployUrl are root relative and not equivalent, unsupported
            // by ng serve
            return null;
        }
        return deployUrl;
...
export default Task.extend({
    run: function(serveTaskOptions: ServeTaskOptions, rebuildDoneCb: any) {
        ...
        let webpackConfig = new NgCliWebpackConfig(
            serveTaskOptions,
            appConfig
        ).buildConfig();
        ...
        let clientAddress = `${
            serveTaskOptions.ssl ? "https" : "http"
        }://0.0.0.0:0`;
        ...
        if (serveTaskOptions.liveReload) {
            ...
            try {
                webpackDevServerPath = require.resolve(
                    "webpack-dev-server/client"
                );
            } catch {
                throw new SilentError(
                    "The 'webpack-dev-server' package could not be found."
                );
            }
            let entryPoints = [`${webpackDevServerPath}?${clientAddress}`];
            if (serveTaskOptions.hmr) {
                const webpackHmrLink =
        ...
        `
                ...
                const showWarning = CliConfig.fromProject().get(
                    "warnings.hmrWarning"
                );
                if (showWarning) {
                    ui.writeLine(
                    ...
                    ui.writeLine(
                        oneLine`
            ${yellow(
                "To disable this warning use 'ng set warnings.hmrWarning=false'"
            )}
          `
                    );
                }
          ...
          `
            ...
            ui.writeLine(
                yellow("Live reload is disabled. HMR option ignored.")
            );
        }

        if (!serveTaskOptions.watch) {
            // There's no option to turn off file watching in webpack-dev-server, but we can
            // override the file watcher instead.
        ...
        if (serveTaskOptions.proxyConfig) {
            const proxyPath = path.resolve(
                this.project.root,
                serveTaskOptions.proxyConfig
            );
            if (fs.existsSync(proxyPath)) {
                proxyConfig = require(proxyPath);
            } else {
                const message =
                    "Proxy config file " + proxyPath + " does not exist.";
        ...
        if (!servePath && servePath !== "") {
            const defaultServePath = findDefaultServePath(
                serveTaskOptions.baseHref,
                serveTaskOptions.deployUrl
            );
            const showWarning = CliConfig.fromProject().get(
                "warnings.servePathDefault"
            );
      ...
      `)
        ...
        ui.writeLine(
            chalk.green(oneLine`
      **
      NG Live Development Server is listening on ${serveTaskOptions.host}:${
    ...
    `)
        ...
        return new Promise((_resolve, reject) => {
            ...
            // Connections will end after ~5 seconds (arbitrary), often not letting the full
            // download of large pieces of content, such as a vendor javascript file.  This
            // results in browsers throwing a "net::ERR_CONTENT_LENGTH_MISMATCH" error.
