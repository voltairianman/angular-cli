...
const hiddenOptions = ["name", "path", "source-dir", "app-root", "link-cli"];
...
export default Task.extend({
    run: function({
        ...
        collectionName,
        nonSchematicOptions,
    }: SchematicGetHelpOptions): Promise<string[]> {
        const SchematicGetOptionsTask = require("./schematic-get-options")
            .default;
        ...
        return Promise.all([
            ...
            ([availableOptions, nonSchematicOptions]: [
                SchematicAvailableOptions[],
                any[]
            ]) => {
                const output: string[] = [];
                ...
                if (availableOptions === null) {
                    output.push(
                        green(
                            "This schematic accept additional options, but did not provide documentation."
                        )
                    );
                }
