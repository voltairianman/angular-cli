    ...
    run: function(options: LintTaskOptions) {
        ...
        const result = lintConfigs
            .map(config => {
                ...
                // TSLint < 5.5 has a bug with fix and project used in combination. previous
                // behavior of typeCheck option is maintained for those versions
            ...
            .reduce(
                (total, current) => {
                    const failures = current.failures.filter(
                        cf => !total.failures.some(ef => ef.equals(cf))
                    );
        ...
        if (!options.silent) {
            ...
            if (!Formatter) {
                throw new SilentError(
                    chalk.red(`Invalid lint format "${options.format}".`)
                );
            }
        ...
        if (result.warningCount > 0 && !options.silent) {
            ui.writeLine(
                chalk.yellow("Lint warnings found in the listed files.")
            );
        }
        ...
        return options.force || result.errorCount === 0
            ? Promise.resolve(0)
            : Promise.resolve(2);
    },
});
...
function getFilesToLint(
    ...
    const providedFiles =
        lintConfig.files && normalizeArrayOption(lintConfig.files);
    const ignore =
        lintConfig.exclude && normalizeArrayOption(lintConfig.exclude);
    ...
    if (ignore && ignore.length > 0) {
        ...
        programFiles = programFiles.filter(
            file => !ignoreMatchers.some(matcher => matcher.match(file))
        );
    }
...
function getFileContents(
    ...
    if (program) {
        if (program.getSourceFile(file) == undefined) {
            const message = `File '${file}' is not part of the TypeScript project '${
                config.project
            }'.`;
    ...
    // NOTE: The tslint CLI checks for and excludes MPEG transport streams; this
    // does not.
    try {
