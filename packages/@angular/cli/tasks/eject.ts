...
class JsonWebpackSerializer {
    ...
    public variables: {
        ...
        genDirNodeModules: `path.join(process.cwd(), '${
            this._appRoot
        }', '$$_gendir', 'node_modules')`,
    };
    ...
    private _globCopyWebpackPluginSerialize(value: any): any {
        ...
        return { patterns, globOptions: this._globReplacer(globOptions) };
    }
    ...
    private _htmlWebpackPlugin(value: any) {
        ...
        this.variables["entryPoints"] = JSON.stringify(
            chunksSortMode.entryPoints
        );
        return Object.assign({}, value.options, {
    ...
    private _pluginsReplacer(plugins: any[]) {
        return plugins.map(plugin => {
            ...
            const serializer = (args: any) =>
                JSON.stringify(args, (k, v) => this._replacer(k, v), 2);

            switch (plugin.constructor) {
                case ProgressPlugin:
                    this.variableImports["webpack/lib/ProgressPlugin"] =
                        "ProgressPlugin";
                    break;
                ...
                case angularCliPlugins.InsertConcatAssetsWebpackPlugin:
                    args = this._insertConcatAssetsWebpackPluginSerialize(
                        plugin
                    );
                    this._addImport(
                        "@angular/cli/plugins/webpack",
                ...
                case ExtractTextPlugin:
                    ...
                    this.variableImports["extract-text-webpack-plugin"] =
                        "ExtractTextPlugin";
                    break;
                case CircularDependencyPlugin:
                    this.variableImports["circular-dependency-plugin"] =
                ...
                case HtmlWebpackPlugin:
                    ...
                    this.variableImports["html-webpack-plugin"] =
                        "HtmlWebpackPlugin";
                    break;
                ...
                case UglifyJSPlugin:
                    ...
                    this.variableImports["uglifyjs-webpack-plugin"] =
                        "UglifyJsPlugin";
                    break;
                case SubresourceIntegrityPlugin:
                    this.variableImports["webpack-subresource-integrity"] =
                        "SubresourceIntegrityPlugin";
                    break;
                default:
                    if (
                        plugin.constructor.name == "AngularServiceWorkerPlugin"
                    ) {
                        this._addImport(
                            "@angular/service-worker/build/webpack",
                            plugin.constructor.name
                        );
                    } else if (plugin["copyWebpackPluginPatterns"]) {
                        // CopyWebpackPlugin doesn't have a constructor nor save args.
                        this.variableImports["copy-webpack-plugin"] =
                            "CopyWebpackPlugin";
                        const patternOptions = plugin[
                            "copyWebpackPluginPatterns"
                        ].map((pattern: any) => {
                            if (pattern.context) {
                                pattern.context = path.relative(
                                    process.cwd(),
                                    pattern.context
                                );
                            }
                            if (pattern.from && pattern.from.glob) {
                                pattern.from.glob = path.relative(
                                    process.cwd(),
                                    pattern.from.glob
                                );
                            }
                        ...
                        const optionsSerialized =
                            serializer(plugin["copyWebpackPluginOptions"]) ||
                            "undefined";
    ...
    private _resolveReplacer(value: any) {
        ...
        return Object.assign({}, value, {
            ...
            modules:
                value.modules &&
                value.modules.map(
                    (x: string) => "./" + path.relative(this._root, x)
                ),
        });
    }
    ...
    private _outputReplacer(value: any) {
        return Object.assign({}, value, {
            path: this._relativePath(
                "process.cwd()",
                path.relative(this._root, value.path)
            ),
        });
    }
        ...
        if (typeof loader == "string") {
            ...
            if (loader.loader === "postcss-loader") {
                if (!this._postcssProcessed) {
                    ...
                    Object.keys(args.imports).forEach(key =>
                        this._addImport(key, args.imports[key])
                    );
                    Object.keys(args.variableImports).forEach(
                        key =>
                            (this.variableImports[key] =
                                args.variableImports[key])
                    );
                    Object.keys(args.variables).forEach(key => {
                        ...
                        if (value === process.cwd()) {
                            ...
                            this.variables[key] =
                                "process.cwd() + " +
                                JSON.stringify(
                                    value.substr(process.cwd().length)
                                );
                        } else {
    ...
    private _ruleReplacer(value: any) {
        ...
        if (value[pluginArgs]) {
            ...
            options.use = options.use.map((loader: any) =>
                this._loaderReplacer(loader)
            );
            ...
            return {
                ...
                loaders: this._escape(
                    `ExtractTextPlugin.extract(${JSON.stringify(
                        options,
                        null,
                        2
                    )})`
        ...
        if (value.loaders) {
            value.loaders = value.loaders.map((loader: any) =>
                this._loaderReplacer(loader)
            );
        }
        if (value.loader) {
        ...
        if (value.use) {
            if (Array.isArray(value.use)) {
                value.use = value.use.map((loader: any) =>
                    this._loaderReplacer(loader)
                );
            } else {
    ...
    private _moduleReplacer(value: any) {
        return Object.assign({}, value, {
            rules:
                value.rules &&
                value.rules.map((x: any) => this._ruleReplacer(x)),
        });
    }

    private _globReplacer(value: any) {
        return Object.assign({}, value, {
            cwd: this._relativePath(
                "process.cwd()",
                path.relative(this._root, value.cwd)
            ),
        });
    }
    ...
    private _licenseReplacer(value: any) {
        return Object.assign({}, value, {
            outputTemplate: this._relativePath(
                "process.cwd()",
                path.relative(this._root, value.outputTemplate)
            ),
        });
    }
    ...
    serialize(config: any): string {
        ...
        config["resolveLoader"] = this._resolveReplacer(
        ...
        return JSON.stringify(config, (k, v) => this._replacer(k, v), 2)
            ...
            .replace(
                /(\s*)(.*?)"\uFF02(.*?)\uFF02"(,?).*/g,
                (_, indent, key, value, comma) => {
                    const ctor = JSON.parse(`"${value}"`)
                        .split(/\n+/g)
                        .join(indent);
    ...
    generateVariables(): string {
        let variableOutput = "";
        Object.keys(this.variableImports).forEach((key: string) => {
            const [module, name] = key.split(/\./);
            variableOutput +=
                `const ${this.variableImports[key]} = require` +
                `('${module}')`;
            if (name) {
                variableOutput += "." + name;
            }
        ...
        Object.keys(this.imports).forEach((key: string) => {
            const [module, name] = key.split(/\./);
            variableOutput +=
                `const { ${this.imports[key].join(", ")} } = require` +
                `('${module}')`;
            if (name) {
                variableOutput += "." + name;
            }
...
export default Task.extend({
    run: function(runTaskOptions: EjectTaskOptions) {
        ...
        if (appConfig.platform === "server") {
            throw new SilentError(
                "ng eject for platform server applications is coming soon!"
            );
        }

        const webpackConfig = new NgCliWebpackConfig(
            runTaskOptions,
            appConfig
        ).buildConfig();
        const serializer = new JsonWebpackSerializer(
            process.cwd(),
            outputPath,
            appConfig.root
        );
        ...
        return (
            Promise.resolve()
                ...
                // Read the package.json and update it to include npm scripts. We do this first
                // so that if an error already exists
            ...
            `
                        ...
                        if (
                            scripts["pree2e"] &&
                            scripts["pree2e"] !== pree2eNpmScript
                        ) {
                            throw new SilentError(
                                oneLine`
            ...
            `
                    ...
                    if (
                        ...
                        runTaskOptions.target === "production" &&
                        usesServiceWorker(project.root) &&
                        !!runTaskOptions.serviceWorker
                    ) {
                        packageJson["scripts"]["build"] +=
                            " && npm run sw-config && npm run sw-copy";
                        packageJson["scripts"][
                            "sw-config"
                        ] = `ngsw-config ${outputPath} src/ngsw-config.json`;
                        packageJson["scripts"][
                            "sw-copy"
