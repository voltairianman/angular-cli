// Remove .js files from entry points consisting entirely of
// .css|scss|sass|less|styl. To be used together with ExtractTextPlugin.
    ...
    apply(compiler: any): void {
        compiler.plugin("compilation", function(compilation: any) {
            ...
            for (let entryPoint of Object.keys(entryPoints)) {
                ...
                entryFiles =
                    entryFiles instanceof Array ? entryFiles : [entryFiles];
                if (
                    entryFiles.every(
                        (el: string) =>
            ...
            compilation.plugin("after-seal", (callback: any) => {
                compilation.chunks
                    .filter(
                        (chunk: any) => cssOnlyChunks.indexOf(chunk.name) !== -1
                    )
                    .forEach((chunk: any) => {
                        let newFiles: string[] = [];
            ...
            // Remove scripts tags with a css file as source, because HtmlWebpackPlugin will
            // use a css file as a script for chunks without js files.
            compilation.plugin(
